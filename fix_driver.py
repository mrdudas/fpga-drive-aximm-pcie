...
# 1. Add nvme_map_bar_fallback right before nvme_init
target_find = "int nvme_init(struct udevice *udev)"
...
if "nvme_map_bar_fallback" not in text:
    text = text.replace(target_find, fallback_code + target_find)

# 2. Modify nvme_init to use the fallback and debug prints
orig_init_match = """ndev->udev = udev;
INIT_LIST_HEAD(&ndev->namespaces);
if (readl(&ndev->bar->csts) == -1) {"""

new_init_code = """ndev->udev = udev;
INIT_LIST_HEAD(&ndev->namespaces);
...
if orig_init_match in text:
    text = text.replace(orig_init_match, new_init_code)
else:
    print("Could not match orig_init_match!")
