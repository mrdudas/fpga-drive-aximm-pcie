...
#define NVME_LINK_LOG_THROTTLE_MS	2000

static int nvme_pci_endpoint_present(struct nvme_dev *dev)
{
	u32 vid_did;
	int ret;
	...
	ret = dm_pci_read_config32(dev->udev, PCI_VENDOR_ID, &vid_did);
	if (ret)
		return 0;

	if (vid_did == 0 || vid_did == 0xffffffff)
		return 0;
...
static int nvme_mmio_guard(struct nvme_dev *dev, const char *op)
{
	int link_up;
	ulong now_ms;
	...
	link_up = nvme_pci_endpoint_present(dev);
	now_ms = get_timer(0);

	if (!dev->link_state_valid || dev->last_link_up != link_up) {
		dev->link_state_valid = 1;
		dev->last_link_up = link_up;
		dev->last_link_log_ms = now_ms;
		printf("NVME: endpoint link %s (%s)\n",
		       link_up ? "UP" : "DOWN",
		       op ? op : "mmio");
	} else if (!link_up &&
		   get_timer(dev->last_link_log_ms) >= NVME_LINK_LOG_THROTTLE_MS) {
		dev->last_link_log_ms = now_ms;
		printf("NVME: endpoint still DOWN (%s), MMIO suppressed\n",
		       op ? op : "mmio");
	}

	return link_up ? 0 : -ENOLINK;
}

static int nvme_mmio_readl(struct nvme_dev *dev, const u32 __iomem *addr,
			   u32 *val, const char *op)
{
	int ret = nvme_mmio_guard(dev, op);
...
static int nvme_mmio_writel(struct nvme_dev *dev, u32 val, u32 __iomem *addr,
			    const char *op)
{
	int ret = nvme_mmio_guard(dev, op);
...
static int nvme_writeq_safe(struct nvme_dev *dev, const u64 val,
			    __le64 volatile *regs, const char *op)
{
	...
	ret = nvme_mmio_writel(dev, val_lo, ptr, op);
	if (ret)
		return ret;

	return nvme_mmio_writel(dev, val_hi, ptr + 1, op);
}
...
static int nvme_wait_csts(struct nvme_dev *dev, u32 mask, u32 val)
{
	...
	while (get_timer(start) < timeout) {
		ret = nvme_mmio_readl(dev, &dev->bar->csts, &csts, "wait_csts");
		if (ret)
			return ret;
		if ((csts & mask) == val)
			return 0;
...
static int nvme_submit_cmd(struct nvme_queue *nvmeq, struct nvme_command *cmd)
{
	...
	if (++tail == nvmeq->q_depth)
		tail = 0;
	if (nvme_mmio_writel(nvmeq->dev, tail, nvmeq->q_db, "sq_db"))
		return -ENOLINK;
	nvmeq->sq_tail = tail;
				...
				u32 *result, unsigned timeout)
{
	...
	if (status) {
		...
		if (nvme_mmio_writel(nvmeq->dev, head,
		    nvmeq->q_db + nvmeq->dev->db_stride, "cq_db"))
			return -ENOLINK;
		nvmeq->cq_head = head;
...
static int nvme_enable_ctrl(struct nvme_dev *dev)
{
	...
	if (nvme_mmio_writel(dev, dev->ctrl_config, &dev->bar->cc, "cc_enable"))
		return -ENOLINK;
...
static int nvme_disable_ctrl(struct nvme_dev *dev)
{
	dev->ctrl_config &= ~NVME_CC_SHN_MASK;
	dev->ctrl_config &= ~NVME_CC_ENABLE;
	if (nvme_mmio_writel(dev, dev->ctrl_config, &dev->bar->cc, "cc_disable"))
		return -ENOLINK;
...
static int nvme_shutdown_ctrl(struct nvme_dev *dev)
{
	...
	if (nvme_mmio_writel(dev, dev->ctrl_config, &dev->bar->cc, "cc_shutdown"))
		return -ENOLINK;
...
static int nvme_configure_admin_queue(struct nvme_dev *dev)
{
	...
	if (nvme_mmio_writel(dev, aqa, &dev->bar->aqa, "aqa"))
		goto free_nvmeq;
	if (nvme_writeq_safe(dev, (ulong)nvmeq->sq_cmds, &dev->bar->asq, "asq"))
		goto free_nvmeq;
	if (nvme_writeq_safe(dev, (ulong)nvmeq->cqes, &dev->bar->acq, "acq"))
		goto free_nvmeq;
...
int nvme_get_log(struct nvme_dev *dev, u8 lid, u8 lsp, u8 rae,
		 u32 len, dma_addr_t dma_addr)
{
	struct nvme_command c;
	u32 page_size = dev->page_size;
	int offset = dma_addr & (page_size - 1);
	...
	if (length <= 0) {
		...
		dma_addr += (page_size - offset);
		c.common.prp2 = cpu_to_le64(dma_addr);
	}

	/*
	 * cdw10:
	 *   [31:16] NUMDL (number of dwords - 1, lower 16 bits)
	 *   [14:8]  LSP   (log specific field)
	 *   [7:0]   LID   (log page identifier)
	 */
	u32 numd = (len / 4) - 1;
	c.common.cdw10[0] = cpu_to_le32((numd & 0xFFFF) << 16 | (rae << 15) | (lsp << 8) | lid);
	
	/*
	 * cdw11:
	 *   [31:16] LPOOL (log page offset lower)
	 *   [15:0]  NUMDU (number of dwords - 1, upper 16 bits)
	 */
	c.common.cdw10[1] = cpu_to_le32((numd >> 16) & 0xFFFF);
	
	c.common.cdw10[2] = 0; /* LPOU (Log Page Offset Upper) */
...
ret = dm_pci_read_config32(udev, 0x10, &bar0);
if (ret)
	printf("NVME DBG: BAR0 read failed ret=%d\n", ret);
else {
	bar_phys = (u64)(bar0 & 0xfffffff0);
	if ((bar0 & 0x6) == 4) {
		if (!dm_pci_read_config32(udev, 0x14, &bar1))
			bar_phys |= ((u64)bar1 << 32);
	}

	printf("NVME DBG: BAR decode BAR0=0x%08x BAR1=0x%08x phys=0x%llx\n",
	       bar0, bar1, bar_phys);

	if (bar_phys) {
		phys_addr_t bar_map_base = (phys_addr_t)(bar_phys & ~0x1FFFFFULL);
		mmu_set_region_dcache_behaviour(bar_map_base, 0x200000, DCACHE_OFF);
		printf("NVME DBG: MMU ATTR BAR probe range [0x%llx..0x%llx] (2MB aligned) set to DCACHE_OFF\n",
		       (u64)bar_map_base, (u64)bar_map_base + 0x200000 - 1);
	}
}
...
printf("NVME DBG: -> Probe MMIO readl CAP(low) at %p ...\n", &ndev->bar->cap);
{
	u32 cap_lo = 0;
	if (nvme_mmio_readl(ndev, (void __iomem *)&ndev->bar->cap, &cap_lo, "probe_cap")) {
		ret = -ENOLINK;
		goto free_nvme;
	}
	printf("NVME DBG: <- CAP(low)=0x%08x\n", cap_lo);
}

printf("NVME DBG: -> Probe MMIO readl CSTS at %p ...\n", &ndev->bar->csts);


{
	u32 csts = 0;
	if (nvme_mmio_readl(ndev, &ndev->bar->csts, &csts, "probe_csts") ||
	    csts == 0xffffffff) {
		ret = -ENODEV;
	...
	if (nvme_mmio_guard(ndev, "read_cap_qword")) {
		ret = -ENOLINK;
		goto free_queue;
	}
